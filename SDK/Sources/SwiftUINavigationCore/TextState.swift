...
public struct TextState: Equatable, Hashable, Sendable {
    ...
    public enum FontWidth: String, Equatable, Hashable, Sendable {
        ...
        @available(iOS 16, macOS 13, tvOS 16, watchOS 9, *) var toSwiftUI: SwiftUI.Font.Width {
            switch self {
    ...
    public enum LineStylePattern: String, Equatable, Hashable, Sendable {
        ...
        @available(iOS 16, macOS 13, tvOS 16, watchOS 9, *) var toSwiftUI: SwiftUI.Text.LineStyle.Pattern {
            switch self {
            case .dash: .dash
    ...
    fileprivate enum Storage: Equatable, Hashable, @unchecked Sendable {
        ...
        static func == (lhs: Self, rhs: Self) -> Bool {
            ...
            case let (.localized(lk, lt, lb, lc), .localized(rk, rt, rb, rc)):
                lk.formatted(tableName: lt, bundle: lb, comment: lc)
            ...
            case let (.localized(key, tableName, bundle, comment), .verbatim(string)),
                ...
                key.formatted(tableName: tableName, bundle: bundle, comment: comment) == string
...
extension TextState {
    public enum AccessibilityTextContentType: String, Equatable, Hashable, Sendable {
        ...
        @available(iOS 15, macOS 12, tvOS 15, watchOS 8, *) var toSwiftUI: SwiftUI.AccessibilityTextContentType {
            switch self {
    ...
    public enum AccessibilityHeadingLevel: String, Equatable, Hashable, Sendable {
        ...
        @available(iOS 15, macOS 12, tvOS 15, watchOS 8, *) var toSwiftUI: SwiftUI.AccessibilityHeadingLevel {
            switch self {
            case .h1: .h1
            case .h2: .h2
            case .h3: .h3
...
extension TextState {
    ...
    public func accessibilityLabel(_ string: some StringProtocol) -> Self {
        var self = self
        `self`.modifiers.append(.accessibilityLabel(.init(string)))
        return `self`
    }
...
extension Text {
    public init(_ state: TextState) {
        let text: Text = switch state.storage {
        case let .concatenated(first, second):
            Text(first) + Text(second)
        case let .localized(content, tableName, bundle, comment):
            .init(content, tableName: tableName, bundle: bundle, comment: comment)
        case let .verbatim(content):
            .init(verbatim: content)
        }
        self = state.modifiers.reduce(text) { text, modifier in
            ...
            case let .strikethrough(isActive, pattern, color):
                if #available(iOS 16, macOS 13, tvOS 16, watchOS 9, *), let pattern {
...
extension TextState: CustomDumpRepresentable {
    public var customDumpValue: Any {
        func dumpHelp(_ textState: Self) -> String {
            var output: String = switch textState.storage {
            case let .concatenated(lhs, rhs):
                dumpHelp(lhs) + dumpHelp(rhs)
            case let .localized(key, tableName, bundle, comment):
                key.formatted(tableName: tableName, bundle: bundle, comment: comment)
            case let .verbatim(string):
                string
            }
