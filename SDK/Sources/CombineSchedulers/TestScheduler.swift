...
public final class TestScheduler<SchedulerTimeType, SchedulerOptions>:
    ...
    public func advance(to instant: SchedulerTimeType) {
        while lock.sync(operation: { self.now }) <= instant {
            lock.lock()
    ...
    /// ```
    public func run() {
        while let date = lock.sync(operation: { self.scheduled.first?.date }) {
            advance(by: lock.sync { self.now.distance(to: date) })
        }
    }
    ...
    public func run() async {
        await Task.megaYield()
        while let date = lock.sync(operation: { self.scheduled.first?.date }) {
            await advance(by: lock.sync { self.now.distance(to: date) })
        }
    }
    ...
    public func schedule(
        ...
        let sequence = lock.sync { self.nextSequence() }
        ...
        lock.sync { self.scheduled.append((sequence, date, scheduleAction(for: date))) }
    ...
    public func schedule(
        ...
        lock.sync { self.scheduled.append((self.nextSequence(), date, action)) }
    }

    public func schedule(options _: SchedulerOptions?, _ action: @escaping () -> Void) {
        lock.sync { self.scheduled.append((self.nextSequence(), self.now, action)) }
    }
