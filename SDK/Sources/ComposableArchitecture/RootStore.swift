...
public final class RootStore {
    ...
    init<State>(
        initialState: State,
        reducer: some Reducer<State, some Any>
    ) {
    ...
    func send(_ action: Any, originatingFrom originatingAction: Any? = nil) -> Task<Void, Never>? {
        func open<State, Action>(reducer: some Reducer<State, Action>) -> Task<Void, Never>? {
            ...
            isSending = true
            var currentState = state as! State
            ...
            while index < bufferedActions.endIndex {
                defer { index += 1 }
                let action = bufferedActions[index] as! Action
...
#else
@_transparent
func threadCheck(status _: ThreadCheckStatus) {}
#endif
