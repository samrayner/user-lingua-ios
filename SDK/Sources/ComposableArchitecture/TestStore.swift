...
public final class TestStore<State, Action> {
    ...
    public func finish(
        ...
        assertNoReceivedActions(file: file, line: line)
        ...
        assertNoSharedChanges(file: file, line: line)
    }
    ...
    private func assertNoReceivedActions(file: StaticString, line: UInt) {
        if !reducer.receivedActions.isEmpty {
            let actions = reducer.receivedActions
                .map(\.action)
    ...
    private func assertNoSharedChanges(file: StaticString, line: UInt) {
        ...
        if sharedChangeTracker.hasChanges, let stateType = State.self as? any Equatable.Type {
            ...
            open(stateType)
            sharedChangeTracker.resetChanges()
        }
    }
...
extension TestStore where State: Equatable {
    ...
    private func expectedStateShouldMatch(
        ...
        try sharedChangeTracker.assert {
...
extension TestStore where State: Equatable, Action: Equatable {
    ...
    public func receive(
        ...
        await XCTFailContext.$current.withValue(XCTFailContext(file: file, line: line)) {
            ...
            _ = self._receive(expectedAction, assert: updateStateToExpectedResult, file: file, line: line)
            await Task.megaYield()
        }
    }
}
...
extension TestStore where State: Equatable {
    ...
    private func _receive(
        _ actionCase: AnyCasePath<Action, some Any>,
        assert updateStateToExpectedResult: ((inout State) throws -> Void)? = nil,
        file: StaticString = #file,
        line: UInt = #line
    ) {
    ...
    public func receive(
        ...
        await XCTFailContext.$current.withValue(XCTFailContext(file: file, line: line)) {
            guard !self.reducer.inFlightEffects.isEmpty
                ...
                _ = self._receive(isMatching, assert: updateStateToExpectedResult, file: file, line: line)
                return
            }
    ...
    @_disfavoredOverload
    public func receive(
        _ actionCase: CaseKeyPath<Action, some Any>,
        timeout nanoseconds: UInt64? = nil,
        assert updateStateToExpectedResult: ((_ state: inout State) throws -> Void)? = nil,
        file: StaticString = #file,
        line: UInt = #line
    ...
    public func receive(
        ...
        await XCTFailContext.$current.withValue(XCTFailContext(file: file, line: line)) {
            guard !self.reducer.inFlightEffects.isEmpty
                ...
                _ = self._receive(actionCase, assert: updateStateToExpectedResult, file: file, line: line)
                return
            }
            await self.receiveAction(
                matching: { actionCase.extract(from: $0) != nil },
    ...
    private func receiveAction(
        ...
        if !predicate(receivedAction) {
            let receivedActionLater = reducer.receivedActions
                .contains(where: { _, _ in predicate(receivedAction) })
            XCTFailHelper(
                """
    ...
    private func receiveAction(
        ...
        while !Task.isCancelled {
            ...
            switch exhaustivity {
            case .on:
                guard reducer.receivedActions.isEmpty
                else { return }
            case .off:
                guard !reducer.receivedActions.contains(where: { predicate($0.action) })
                else { return }
            }
...
extension TestStore where State: Equatable {
    ...
    public func send(
        ...
        await send(action(), assert: updateStateToExpectedResult, file: file, line: line)
    }
    ...
    public func send<Value>(
        ...
        await send(action(value), assert: updateStateToExpectedResult, file: file, line: line)
    }
}
...
extension TestStore {
    ...
    public func skipReceivedActions(
        ...
        await Task.megaYield()
        _ = _skipReceivedActions(strict: strict, file: file, line: line)
    }
    ...
    private func _skipReceivedActions(
        ...
        if strict && reducer.receivedActions.isEmpty {
        ...
        var actions = ""
        if reducer.receivedActions.count == 1 {
            customDump(reducer.receivedActions[0].action, to: &actions)
        } else {
            customDump(reducer.receivedActions.map { $0.action }, to: &actions)
        }
        XCTFailHelper(
            """
            \(reducer.receivedActions.count) received action\
            \(reducer.receivedActions.count == 1 ? " was" : "s were") skipped:

            \(actions)
            """,
            overrideExhaustivity: exhaustivity == .on
    ...
    public func skipInFlightEffects(
        ...
        await Task.megaYield()
        _ = _skipInFlightEffects(strict: strict, file: file, line: line)
    }
    ...
    fileprivate func _skipInFlightEffects(
        ...
        if strict && reducer.inFlightEffects.isEmpty {
            XCTFail("There were no in-flight effects to skip.")
            return
        }
        guard !reducer.inFlightEffects.isEmpty
        else { return }

        var actions = ""
        if reducer.inFlightEffects.count == 1 {
            customDump(reducer.inFlightEffects.first!.action.origin.action, to: &actions)
        } else {
            customDump(reducer.inFlightEffects.map { $0.action.origin.action }, to: &actions)
        }
    ...
    public func finish(
        ...
        do {
            try await withThrowingTaskGroup(of: Void.self) { group in
                group.addTask { await rawValue?.cancellableValue }
                group.addTask {
...
class TestReducer<State, Action>: Reducer {
    ...
    func reduce(into state: inout State, action: TestAction) -> Effect<TestAction> {
        ...
        let reducer = base.dependency(\.self, dependencies)
...
extension TestStore {
    ...
    public func receive(
        _: Action,
        assert _: ((_ state: inout State) throws -> Void)? = nil,
