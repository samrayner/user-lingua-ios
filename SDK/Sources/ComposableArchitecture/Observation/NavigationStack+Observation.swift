...
extension NavigationLink where Destination == Never {
    ...
    @_disfavoredOverload
    public init<P>(
        _ title: some StringProtocol, state: P?, fileID: StaticString = #fileID, line: UInt = #line
    )
        where Label == _NavigationLinkStoreContent<P, Text> {
...
extension Store {
    ...
    public subscript<ElementState, ElementAction>(
        ...
        set {
            ...
            guard newCount != currentState.count else {
                runtimeWarn(
                    """
            ...
            if newCount > currentState.count, let component = newValue.last {
                send(.push(id: component.id, state: component.element))
            } else {
                send(.popFrom(id: currentState.ids[newCount]))
            }
        }
    }
}
...
extension StackState {
    ...
    public struct PathView: MutableCollection, RandomAccessCollection,
        ...
        public subscript(position: Int) -> Component {
            _read {
                yield Component(id: base.ids[position], element: base[position])
            }
            _modify {
                let id = base.ids[position]
                var component = Component(id: id, element: base[position])
                yield &component
                base[id: id] = component.element
            }
            set {
                base[id: newValue.id] = newValue.element
            }
        }
