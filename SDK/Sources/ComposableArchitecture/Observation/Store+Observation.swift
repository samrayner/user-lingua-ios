...
extension Store where State: ObservableState {
    var observableState: State {
        _$observationRegistrar.access(self, keyPath: \.currentState)
        return currentState
    }
...
extension Store where State: ObservableState {
    ...
    public func scope<ChildState, ChildAction>(
        ...
        return scope(
            id: id(state: state.appending(path: \.!), action: action),
            state: ToState {
...
extension Store where State: ObservableState {
    ...
    public subscript<ChildState, ChildAction>(
        ...
        get {
            ...
            #else
            scope(state: state, action: action.appending(path: \.presented))
            #endif
        }
        set {
            if newValue == nil, self.state[keyPath: state] != nil, !_isInvalidated() {
                send(action(.dismiss))
...
func uncachedStoreWarning(_ store: Store<some Any, some Any>) -> String {
    """
