...
public struct ObservationStateRegistrar: Sendable {
    ...
    #if !os(visionOS)
    @usableFromInline let registrar = PerceptionRegistrar()
    #else
    @usableFromInline let registrar = ObservationRegistrar()
    #endif
    public init() {}
...
extension ObservationStateRegistrar: Equatable, Hashable, Codable {
    ...
    public init(from _: Decoder) throws { self.init() }
...
extension ObservationStateRegistrar {
    ...
    public func willModify<Subject: Observable, Member>(
        _: Subject,
        keyPath _: KeyPath<Subject, Member>,
