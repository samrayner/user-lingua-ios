...
public final class Store<State, Action> {
    ...
    @_spi(Internals) public var currentState: State {
    ...
    func scope<ChildState, ChildAction>(
        ...
        if let id, self.canCacheChildren {
    ...
    private init(
        ...
        #if canImport(Perception)
        func subscribeToDidSet<T: ObservableState>(_: T.Type) -> AnyCancellable {
    ...
    @_spi(Internals)
    public func id(
        state: KeyPath<State, some Any>,
        action: CaseKeyPath<Action, some Any>
...
func storeTypeName<State, Action>(of _: Store<State, Action>) -> String {
...
private enum PartialToState<State> {
    ...
    func callAsFunction(_ state: Any) -> State {
        ...
        case let .keyPath(keyPath):
            state[keyPath: keyPath] as! State
        case let .appended(closure, keyPath):
            closure(state)[keyPath: keyPath] as! State
        }
    }
    ...
    func appending<ChildState>(_ state: PartialToState<ChildState>) -> PartialToState<ChildState> {
        ...
        case let (.keyPath(lhs), .keyPath(rhs)):
            .keyPath(lhs.appending(path: rhs)!)
        case let (.closure(lhs), .keyPath(rhs)):
            .appended(lhs, rhs)
        case let (.appended(lhsClosure, lhsKeyPath), .keyPath(rhs)):
            .appended(lhsClosure, lhsKeyPath.appending(path: rhs)!)
        default:
            .closure { state(self($0)) }
        }
    }
}

#if canImport(Perception)
let _isStorePerceptionCheckingEnabled: Bool = if #available(iOS 17, macOS 14, tvOS 17, watchOS 10, *) {
    false
} else {
    true
}
