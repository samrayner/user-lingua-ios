...
public final class FileStorageKey<Value: Codable & Sendable>: PersistenceKey, Sendable {
    ...
    public func load(initialValue: Value?) -> Value? {
        do {
            return try JSONDecoder().decode(Value.self, from: storage.load(url))
        } catch {
            return initialValue
        }
    }
    ...
    public func save(_ value: Value) {
        if workItem.value == nil {
            isSetting.setValue(true)
            try? storage.save(JSONEncoder().encode(value), url)
    ...
    public func subscribe(
        ...
        func setUpSources() {
            cancellable.withValue { [weak self] in
                ...
                let writeCancellable = storage.fileSystemSource(url, [.write]) {
                ...
                let deleteCancellable = storage.fileSystemSource(url, [.delete, .rename]) {
                    didSet(self.load(initialValue: initialValue))
                    setUpSources()
                }
                $0 = AnyCancellable {
        ...
        setUpSources()
        let willResign: (any NSObjectProtocol)? = if let willResignNotificationName {
        ...
        let willTerminate: (any NSObjectProtocol)? = if let willTerminateNotificationName {
            NotificationCenter.default.addObserver(
    ...
    private func performImmediately() {
        guard let workItem = workItem.value
        else { return }
        storage.async(workItem)
        storage.async(
            DispatchWorkItem {
...
public struct FileStorage: Hashable, Sendable {
    ...
    @_spi(Internals)
    public static func fileSystem(queue: DispatchQueue) -> Self {
        Self(
