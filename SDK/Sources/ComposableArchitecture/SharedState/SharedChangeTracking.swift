...
extension Change {
    func assertUnchanged() {
        if let difference = diff(snapshot, reference.value, format: .proportional) {
            XCTFail(
                """
                Tracked changes to '\(reference.description)' but failed to assert: …
...
public final class SharedChangeTracker: Sendable {
    ...
    @_spi(Internals)
    public init() {}
    func resetChanges() { changes.withValue { $0.removeAll() } }
    func assertUnchanged() {
    ...
    func track(_ reference: some Reference<some Any>) {
        if !changes.keys.contains(ObjectIdentifier(reference)) {
            changes.withValue { $0[ObjectIdentifier(reference)] = AnyChange(reference) }
        }
    }
...
extension DependencyValues {
    @_spi(Internals) public var sharedChangeTrackers: Set<SharedChangeTracker> {
        get { self[SharedChangeTrackersKey.self] }
        set { self[SharedChangeTrackersKey.self] = newValue }
    }

    @_spi(Internals) public var sharedChangeTracker: SharedChangeTracker? {
