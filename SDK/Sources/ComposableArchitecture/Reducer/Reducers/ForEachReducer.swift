...
>: Reducer {
    ...
    @usableFromInline let toElementsState: WritableKeyPath<Parent.State, IdentifiedArray<ID, Element.State>>

    @usableFromInline let toElementAction: AnyCasePath<Parent.Action, (id: ID, action: Element.Action)>
    ...
    public func reduce(
        ...
        let elementEffects = reduceForEach(into: &state, action: action)
            ...
            areOrderedSetsDuplicates(idsBefore, idsAfter)
                ...
                : .merge(
                    idsBefore.subtracting(idsAfter).map {
                        ._cancel(
                            id: NavigationID(id: $0, keyPath: toElementsState),
    ...
    func reduceForEach(
        ...
        let navigationID = NavigationID(id: id, keyPath: toElementsState)
        let elementNavigationID = navigationIDPath.appending(navigationID)
        return element
            .dependency(\.navigationIDPath, elementNavigationID)
            .reduce(into: &state[keyPath: toElementsState][id: id]!, action: elementAction)
            .map { toElementAction.embed((id, $0)) }
            ._cancellable(id: navigationID, navigationIDPath: navigationIDPath)
    }
}
