            ...
            AnyCasePath<PresentationAction, PresentationAction<AppendedAction>>(
                ...
                extract: {
                    ...
                    case .dismiss:
                        .dismiss
                    case let .presented(action):
                        action[case: keyPath].map { .presented($0) }
                    }
                }
            )
        }
...
public struct _PresentationReducer<Base: Reducer, Destination: Reducer>: Reducer {
    ...
    public func reduce(into state: inout Base.State, action: Base.Action) -> Effect<Base.Action> {
        let initialPresentationState = state[keyPath: toPresentationState]
        ...
        case let (.some(destinationState), .some(.dismiss)):
            ...
            if navigationIDPath(for: destinationState)
                == state[keyPath: toPresentationState].wrappedValue.map(navigationIDPath(for:)) {
                state[keyPath: toPresentationState].wrappedValue = nil
            }

        case let (.some(destinationState), .some(.presented(destinationAction))):
            let destinationNavigationIDPath = navigationIDPath(for: destinationState)
            destinationEffects = destination
                ...
                .reduce(
                    into: &state[keyPath: toPresentationState].wrappedValue!, action: destinationAction
                )
                .map { toPresentationAction.embed(.presented($0)) }
        ...
        case (.none, .some):
            runtimeWarn(
                """
                An "ifLet" at "\(fileID):\(line)" received a presentation action when \
        ...
        let dismissEffects: Effect<Base.Action> = if presentationIdentityChanged,
                                                     ...
                                                     ?? true {
            ._cancel(navigationID: presentedPath)
        } else {
            .none
        }

        if presentationIdentityChanged, state[keyPath: toPresentationState].wrappedValue == nil {
            state[keyPath: toPresentationState].presentedID = nil
        }

        let presentEffects: Effect<Base.Action>
        if presentationIdentityChanged || state[keyPath: toPresentationState].presentedID == nil,
           let presentationState = state[keyPath: toPresentationState].wrappedValue,
           !isEphemeral(presentationState) {
            let presentationDestinationID = navigationIDPath(for: presentationState)
            state[keyPath: toPresentationState].presentedID = presentationDestinationID
            presentEffects = .concatenate(
                ...
                .publisher { Just(toPresentationAction.embed(.dismiss)) }
            )
    ...
    func navigationIDPath(for state: Destination.State) -> NavigationIDPath {
        navigationIDPath.appending(
            NavigationID(
                base: state,
                keyPath: toPresentationState.appending(path: \.wrappedValue)
            )
        )
    }
}
