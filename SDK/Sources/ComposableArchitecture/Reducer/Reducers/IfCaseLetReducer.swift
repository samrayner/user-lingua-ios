...
public struct _IfCaseLetReducer<Parent: Reducer, Child: Reducer>: Reducer {
    ...
    @usableFromInline let toChildState: AnyCasePath<Parent.State, Child.State>

    @usableFromInline let toChildAction: AnyCasePath<Parent.Action, Child.Action>
    ...
    public func reduce(
        ...
        let childEffects = reduceChild(into: &state, action: action)

        let childIDBefore = toChildState.extract(from: state).map {
            NavigationID(root: state, value: $0, casePath: toChildState)
        }
        let parentEffects = parent.reduce(into: &state, action: action)
        let childIDAfter = toChildState.extract(from: state).map {
            NavigationID(root: state, value: $0, casePath: toChildState)
        }

        let childCancelEffects: Effect<Parent.Action> = if let childElement = childIDBefore, childElement != childIDAfter {
            .cancel(id: childElement)
        } else {
            .none
        }
    ...
    func reduceChild(
        ...
        let childID = NavigationID(root: state, value: childState, casePath: toChildState)
        let newNavigationID = navigationIDPath.appending(childID)
        return child
