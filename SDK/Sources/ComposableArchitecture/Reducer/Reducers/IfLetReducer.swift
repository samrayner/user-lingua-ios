...
public struct _IfLetReducer<Parent: Reducer, Child: Reducer>: Reducer {
    ...
    public func reduce(
        ...
        if childIDAfter == childIDBefore,
           ...
           let childState = state[keyPath: toChildState],
           isEphemeral(childState) {
            state[keyPath: toChildState] = nil
        }

        let childCancelEffects: Effect<Parent.Action> = if let childID = childIDBefore, childID != childIDAfter {
            ._cancel(id: childID, navigationID: navigationIDPath)
        } else {
            .none
        }
    ...
    func reduceChild(
        ...
        guard state[keyPath: toChildState] != nil else {
            runtimeWarn(
                """
        ...
        let navigationID = NavigationID(
            base: state[keyPath: toChildState]!, keyPath: toChildState
        )
        return child
            .dependency(\.navigationIDPath, navigationIDPath.appending(navigationID))
