...
public struct StackState<Element> {
    ...
    public subscript(id id: StackElementID) -> Element? {
        _read { yield _dictionary[id] }
        _modify { yield &_dictionary[id] }
        set {
            switch (ids.contains(id), newValue, _XCTIsTesting) {
    ...
    public mutating func pop(to id: StackElementID) {
        ...
        removeSubrange(index.advanced(by: 1)...)
    }
}

extension StackState: RandomAccessCollection, RangeReplaceableCollection {
    public var startIndex: Int { _dictionary.keys.startIndex }
    public var endIndex: Int { _dictionary.keys.endIndex }
    public func index(after i: Int) -> Int { _dictionary.keys.index(after: i) }
    public func index(before i: Int) -> Int { _dictionary.keys.index(before: i) }
    public subscript(position: Int) -> Element { _dictionary.values[position] }
    public init() {
        self._dictionary = [:]
    }

    public mutating func removeAll(keepingCapacity keepCapacity: Bool = false) {
        _dictionary.removeAll(keepingCapacity: keepCapacity)
    }

    public mutating func replaceSubrange(_ subrange: Range<Int>, with newElements: some Collection<Element>) {
        _dictionary.removeSubrange(subrange)
        for (offset, element) in zip(subrange.lowerBound..., newElements) {
            _dictionary.updateValue(element, forKey: stackElementID.next(), insertingAt: offset)
        }
    }
}
...
extension StackState: CustomStringConvertible {
    public var description: String {
        _dictionary.values.elements.description
    }
}

extension StackState: CustomDebugStringConvertible {
    public var debugDescription: String {
        "\(Self.self)(\(_dictionary.description))"
    }
}

extension StackState: CustomDumpReflectable {
    public var customDumpMirror: Mirror {
        Mirror(self, unlabeledChildren: Array(zip(ids, self)), displayStyle: .dictionary)
    }
}
...
public struct _StackReducer<Base: Reducer, Destination: Reducer>: Reducer {
    ...
    public func reduce(into state: inout Base.State, action: Base.Action) -> Effect<Base.Action> {
        let idsBefore = state[keyPath: toStackState]._mounted
        ...
        case let .element(elementID, destinationAction):
            if state[keyPath: toStackState][id: elementID] != nil {
                let elementNavigationIDPath = navigationIDPath(for: elementID)
                destinationEffects = destination
                    .dependency(
                        \.dismiss,
                    ...
                    .reduce(
                        into: &state[keyPath: toStackState][id: elementID]!,
            ...
            baseEffects = base.reduce(into: &state, action: action)

        case let .popFrom(id):
            destinationEffects = .none
            let canPop = state[keyPath: toStackState].ids.contains(id)
            baseEffects = base.reduce(into: &state, action: action)
            if canPop {
                ...
                runtimeWarn(
                    """
                    A "forEach" at "\(fileID):\(line)" received a "popFrom" action for a missing \
                    element. …

                      ID:
                        \(id)
                      Path IDs:
                        \(state[keyPath: toStackState].ids)
                    """
                )
            }
        ...
        case let .push(id, element):
            destinationEffects = .none
            if state[keyPath: toStackState].ids.contains(id) {
                runtimeWarn(
                    """
                    A "forEach" at "\(fileID):\(line)" received a "push" action for an element it \
                    already contains. …
                ...
                if id.generation > nextID.generation {
                    runtimeWarn(
                        """
                        A "forEach" at "\(fileID):\(line)" received a "push" action with an \
            ...
            state[keyPath: toStackState]._dictionary[id] = element
            baseEffects = base.reduce(into: &state, action: action)
        ...
        let idsAfter = state[keyPath: toStackState].ids
            ...
            areOrderedSetsDuplicates(idsBefore, idsAfter)
                ...
                : .merge(
                    idsBefore.subtracting(idsAfter).map {
                        ._cancel(navigationID: navigationIDPath(for: $0))
                    }
                )
            ...
            areOrderedSetsDuplicates(idsBefore, idsAfter)
                ...
                : .merge(
                    idsAfter.subtracting(idsBefore).map { elementID in
                        let navigationDestinationID = navigationIDPath(for: elementID)
                        return .concatenate(
                            ...
                            .publisher { Just(toStackAction.embed(.popFrom(id: elementID))) }
                        )
        ...
        state[keyPath: toStackState]._mounted = idsAfter
    ...
    private func navigationIDPath(for elementID: StackElementID) -> NavigationIDPath {
        navigationIDPath.appending(
            NavigationID(
                id: elementID,
                keyPath: toStackState
            )
        )
    }
}
