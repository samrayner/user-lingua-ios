...
public struct _OnChangeReducer<Base: Reducer, Value, Body: Reducer>: Reducer
    ...
    @usableFromInline let toValue: (Base.State) -> Value

    @usableFromInline let isDuplicate: (Value, Value) -> Bool

    @usableFromInline let reducer: (Value, Value) -> Body
    ...
    public func reduce(into state: inout Base.State, action: Base.Action) -> Effect<Base.Action> {
        ...
        return isDuplicate(oldValue, newValue)
            ? baseEffects
            : .merge(baseEffects, reducer(oldValue, newValue).reduce(into: &state, action: action))
    }
}
