...
public struct BindingAction<Root>: CasePathable, Equatable, @unchecked Sendable {
    ...
    @usableFromInline let set: @Sendable (inout Root) -> Void
    ...
    var value: AnySendable { _value[0] }
    #else
...
extension BindingAction {
    ...
    /// ```
    public static func ~= (
        keyPath: WritableKeyPath<Root, BindingState<some Any>>,
        bindingAction: Self
    ) -> Bool {
...
extension BindingAction: CustomDumpStringConvertible {
    public var customDumpDescription: String {
        var description = ".set("
        customDump(keyPath, to: &description, maxDepth: 0)
        description.append(", ")
        customDump(value.base, to: &description, maxDepth: 0)
        description.append(")")
        return description
    }
}
