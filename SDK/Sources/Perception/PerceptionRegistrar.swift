...
public struct PerceptionRegistrar: Sendable {
    ...
    #if canImport(Observation)
    @available(iOS 17, macOS 14, tvOS 17, watchOS 10, *) private var registrar: ObservationRegistrar {
        _rawValue.base as! ObservationRegistrar
    }
    #endif

    private var perceptionRegistrar: _PerceptionRegistrar {
        _rawValue.base as! _PerceptionRegistrar
    }
}
...
extension PerceptionRegistrar {
    public func access<Subject: Observable>(
        _ subject: Subject, keyPath: KeyPath<Subject, some Any>
    ) {
    ...
    public func withMutation<Subject: Observable, T>(
        ...
        try registrar.withMutation(of: subject, keyPath: keyPath, mutation)
    }

    public func willSet<Subject: Observable>(
        _ subject: Subject, keyPath: KeyPath<Subject, some Any>
    ) {
        registrar.willSet(subject, keyPath: keyPath)
    }

    public func didSet<Subject: Observable>(
        _ subject: Subject, keyPath: KeyPath<Subject, some Any>
    ) {
        ...
        if #available(iOS 17, macOS 14, tvOS 17, watchOS 10, *),
           let subject = subject as? any Observable {
            func open<S: Observable>(_ subject: S) throws -> T {
                try self.registrar.withMutation(
                    of: subject,
    ...
    public func willSet<Subject: Perceptible, Member>(
        ...
        if #available(iOS 17, macOS 14, tvOS 17, watchOS 10, *),
           let subject = subject as? any Observable {
            ...
            return perceptionRegistrar.willSet(subject, keyPath: keyPath)
        }
        #endif
    }
    ...
    public func didSet<Subject: Perceptible, Member>(
        ...
        if #available(iOS 17, macOS 14, tvOS 17, watchOS 10, *),
           let subject = subject as? any Observable {
            ...
            return perceptionRegistrar.didSet(subject, keyPath: keyPath)
        }
        #endif
    }
}
...
extension PerceptionRegistrar: Codable {
    ...
    public func encode(to _: any Encoder) {
...
extension PerceptionRegistrar {
    ...
    fileprivate func isInSwiftUIBody(file: StaticString, line: UInt) -> Bool {
        perceptionChecks.withCriticalRegion { perceptionChecks in
...
extension String {
    var isGeometryTrailingClosure: Bool {
        contains("(SwiftUI.GeometryProxy) -> ")
    }

    fileprivate var isSuspendingClosure: Bool {
        let fragment = utf8.drop(while: { $0 != .init(ascii: ")") }).dropFirst()
    ...
    fileprivate var demangled: String? {
        utf8CString.withUnsafeBufferPointer { mangledNameUTF8CStr in
