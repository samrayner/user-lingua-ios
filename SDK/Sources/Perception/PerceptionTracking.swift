...
public struct PerceptionTracking: Sendable {
    ...
    public struct _AccessList: Sendable {
        ...
        mutating func addAccess(
            keyPath: PartialKeyPath<some Perceptible>,
    ...
    public static func _installTracking(
        ...
        let values = tracking.list.entries.mapValues {
            ...
            case let (.some(willSetPerceptor), .some(didSetPerceptor)):
                Id.full(
            ...
            case let (.some(willSetPerceptor), .none):
                Id.willSet(
                    $0.addWillSetPerceptor {
                        willSetPerceptor(tracking)
                    }
                )
            case let (.none, .some(didSetPerceptor)):
                Id.didSet(
                    $0.addDidSetPerceptor {
                        didSetPerceptor(tracking)
                    }
    ...
    func install(_ values: [ObjectIdentifier: PerceptionTracking.Id]) {
