...
extension IdentifiedArray { // : MutableCollection
    ...
    public mutating func move(fromOffsets source: IndexSet, toOffset destination: Int) {
        let lowerCount = distance(from: startIndex, to: destination)
        let upperCount = distance(from: destination, to: endIndex)
        _ = _indexedStablePartition(
            count: lowerCount,
            range: startIndex ..< destination,
            by: { source.contains($0) }
        )
        _ = _indexedStablePartition(
            count: upperCount,
            range: destination ..< endIndex,
            by: { !source.contains($0) }
        )
    }
}
