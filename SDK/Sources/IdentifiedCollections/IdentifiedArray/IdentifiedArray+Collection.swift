...
extension IdentifiedArray: Collection {
    @inlinable
    @inline(__always) public var startIndex: Int { _dictionary.keys.startIndex }

    @inlinable
    @inline(__always) public var endIndex: Int { _dictionary.keys.endIndex }
    ...
    public func filter(
        ...
        try .init(
            id: id,
            _id: _id,
            _dictionary: _dictionary.filter { try isIncluded($1) }
        )
    }
}
