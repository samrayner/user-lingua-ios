...
public struct IdentifiedArray<ID, Element> where ID: Hashable {
    ...
    //     key path access.
    @usableFromInline var _id: (Element) -> ID

    @usableFromInline var _dictionary: OrderedDictionary<ID, Element>

    /// A read-only collection view for the ids contained in this array, as an `OrderedSet`.
    ///
    /// - Complexity: O(1)
    @inlinable
    @inline(__always) public var ids: OrderedSet<ID> { _dictionary.keys }

    /// A read-only collection view for the elements contained in this array, as an `Array`.
    ///
    /// - Complexity: O(1)
    @inlinable
    @inline(__always) public var elements: [Element] { _dictionary.values.elements }
    ...
    public subscript(id id: ID) -> Element? {
        ...
        _modify {
            yield &_dictionary[id]
            precondition(
                _dictionary[id].map { _id($0) == id } ?? true,
    ...
    public mutating func remove(_ element: Element) -> Element? {
        _dictionary.removeValue(forKey: _id(element))
    }
    ...
    public mutating func remove(id: ID) -> Element? {
        _dictionary.removeValue(forKey: id)
    }
}
