...
extension View {
    ...
    @ViewBuilder
    public func navigationDestination<Value>(
        unwrapping value: Binding<Value?>,
        @ViewBuilder destination: (Binding<Value>) -> some View
    ) -> some View {
        if requiresBindWorkaround {
            ...
            navigationDestination(isPresented: value.isPresent()) {
...
private struct _NavigationDestinationBindWorkaround<Destination: View>: ViewModifier {
    ...
    public func body(content: Content) -> some View {
        content
            .navigationDestination(isPresented: $isPresentedState) { destination }
