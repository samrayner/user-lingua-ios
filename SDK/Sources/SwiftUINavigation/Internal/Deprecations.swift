...
extension View {
    ...
    public func alert<Enum, Case>(
        ...
        @ViewBuilder actions: (Case) -> some View,
    ...
    public func confirmationDialog<Enum, Case>(
        title: (Case) -> Text,
        titleVisibility: Visibility = .automatic,
        unwrapping enum: Binding<Enum?>,
        case casePath: AnyCasePath<Enum, Case>,
    ...
    public func navigationDestination<Enum, Case>(
        unwrapping enum: Binding<Enum?>,
        case casePath: AnyCasePath<Enum, Case>,
        @ViewBuilder destination: (Binding<Case>) -> some View
    ) -> some View {
...
extension Binding {
    ...
    public func isPresent<Enum>(_ casePath: AnyCasePath<Enum, some Any>) -> Binding<Bool>
        where Value == Enum? {
...
public struct IfCaseLet<Enum, Case, IfContent, ElseContent>: View
    ...
    public var body: some View {
        if let $case = Binding(unwrapping: self.enum, case: casePath) {
            ifContent($case)
        } else {
            elseContent
        }
...
public struct CaseLet<Enum, Case, Content>: View
    ...
    public var body: some View {
        Binding(unwrapping: self.enum.wrappedValue, case: casePath).map(content)
    }
}
...
public struct _ExhaustivityCheckView<Enum>: View {
    ...
    public var body: some View {
        ...
        .onAppear { runtimeWarn(message, file: file, line: line) }
        #else
        EmptyView()
        #endif
    }
