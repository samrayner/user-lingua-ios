...
func extractHelp<Root, Value>(_ embed: @escaping (Value) -> Root) -> (Root) -> Value? {
    ...
    return { root in
        ...
        if let cachedTag, let (isIndirect, associatedValueType) = cachedStrategy {
...
extension Metadata {
    var valueWitnessTable: ValueWitnessTable {
        ValueWitnessTable(
            ptr: ptr.load(fromByteOffset: -pointerSize, as: UnsafeRawPointer.self)
        )
    }
...
public struct EnumMetadata: Metadata {
    ...
    @_spi(Reflection)
    public func tag(of value: some Any) -> UInt32 {
        #if swift(<5.8)
        // NB: Workaround for https://github.com/apple/swift/issues/61708
        guard typeDescriptor.emptyCaseCount + typeDescriptor.payloadCaseCount > 1
        else { return 0 }
        #endif
        return withUnsafePointer(to: value) {
            valueWitnessTable.getEnumTag($0, ptr)
        }
    }
}
...
extension EnumMetadata {
    @_spi(Reflection)
    public func associatedValueType(forTag tag: UInt32) -> Any.Type {
        guard
    ...
    public func caseName(forTag tag: UInt32) -> String? {
        typeDescriptor.fieldDescriptor?.field(atIndex: tag).name
    }
}
...
extension EnumMetadata {
    ...
    @_spi(Reflection)
    public static func project(_ root: some Any) -> Any? {
...
extension GenericArgumentVector {
    func type(atIndex i: Int) -> Any.Type {
        ptr.load(fromByteOffset: i * pointerSize, as: Any.Type.self)
    }
}
