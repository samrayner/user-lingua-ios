...
public struct UnimplementedClock<Duration: DurationProtocol & Hashable>: Clock {
    public struct Instant: InstantProtocol {
        ...
        public func advanced(by duration: Duration) -> Self {
            Self(rawValue: rawValue.advanced(by: duration))
        }
    ...
    public var now: Instant {
        XCTFail("Unimplemented: \(name).now")
        return Instant(rawValue: base.now)
    }

    public var minimumResolution: Duration {
        XCTFail("Unimplemented: \(name).minimumResolution")
        return base.minimumResolution
    }

    public func sleep(until deadline: Instant, tolerance: Duration?) async throws {
        XCTFail("Unimplemented: \(name).sleep")
        try await base.sleep(until: deadline.rawValue, tolerance: tolerance)
    }
}
