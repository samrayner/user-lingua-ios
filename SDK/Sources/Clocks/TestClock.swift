...
public final class TestClock<Duration: DurationProtocol & Hashable>: Clock, @unchecked Sendable {
    ...
    public func sleep(until deadline: Instant, tolerance _: Duration? = nil) async throws {
        ...
        let id = UUID()
        do {
            let stream: AsyncThrowingStream<Never, Error>? = lock.sync {
            ...
            lock.sync { self.suspensions.removeAll(where: { $0.id == id }) }
    ...
    public func checkSuspension() async throws {
        ...
        guard lock.sync(operation: { self.suspensions.isEmpty })
    ...
    public func advance(by duration: Duration = .zero) async {
        await advance(to: lock.sync(operation: { self.now.advanced(by: duration) }))
    }
    ...
    public func advance(to deadline: Instant) async {
        while lock.sync(operation: { self.now <= deadline }) {
            await Task.megaYield()
            let `return` = {
                self.lock.lock()
